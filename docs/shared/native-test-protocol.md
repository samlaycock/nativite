...
after the WebView test runtime has already loaded and emits `webview.ready`
immediately after receiving `harness.register.result`. Readiness is not a
`harness.register` field and cannot be piggybacked on registration. Otherwise
the coordinator must enter `webview_loading` after registration and wait for
the explicit `webview.ready` event before marking the session `ready`.
...
{
  "testUrl": "http://127.0.0.1:5173/__nativite_test__",
  "documentId": "doc-01JVVH92B7R9Y7M5GSPY4CK4ZX",
  "readyAt": "2026-05-15T09:00:01.250Z"
}
```

`documentId` is an opaque string generated by the WebView test runtime once for
each top-level document load before it signals readiness. It must be stable for
the lifetime of that document, unique within the active session, and must change
after a top-level navigation or reload. Native platform navigation identifiers
