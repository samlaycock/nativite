...
- `sessionId` and `sessionToken`: generated by `nativite test` for the local
  coordinator/harness run. The token is required for native harness registration
  and `nativite/test` command helper requests. Generated provider option JSON
  excludes `sessionToken`; the provider reads it from
...
- `openPage(sessionId, url, options)` sends an `open-page` coordinator command
  with the Vitest Browser Mode page URL, session id, authenticated request
  token, and native harness launch options.
